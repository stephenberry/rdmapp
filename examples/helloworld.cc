...
rdmapp::task<void> handle_qp(std::shared_ptr<rdmapp::qp> qp)
{
   ...
   auto local_mr = std::make_shared<rdmapp::local_mr>(qp->pd_ptr()->reg_mr(&buffer[0], sizeof(buffer)));
   auto local_mr_serialized = local_mr->serialize();
   co_await qp->send(local_mr_serialized.data(), local_mr_serialized.size());
   std::cout << "Sent mr addr=" << local_mr->addr() << " length=" << local_mr->length() << " rkey=" << local_mr->rkey()
             << " to client" << std::endl;
   ...
   /* Atomic */
   uint64_t counter = 42;
   auto counter_mr = std::make_shared<rdmapp::local_mr>(qp->pd_ptr()->reg_mr(&counter, sizeof(counter)));
   auto counter_mr_serialized = counter_mr->serialize();
   co_await qp->send(counter_mr_serialized.data(), counter_mr_serialized.size());
   std::cout << "Sent mr addr=" << counter_mr->addr() << " length=" << counter_mr->length()
...
rdmapp::task<void> client(rdmapp::connector& connector)
{
   ...
   std::cout << "Received " << n << " bytes from server: " << buffer << std::endl;
   ...
   std::cout << "Received mr addr=" << remote_mr.addr << " length=" << remote_mr.length << " rkey=" << remote_mr.rkey
   ...
   std::cout << "Received mr addr=" << counter_mr.addr << " length=" << counter_mr.length << " rkey=" << counter_mr.rkey
             << " from server" << std::endl;
   uint64_t counter = 0;
   co_await qp->fetch_and_add(counter_mr, &counter, sizeof(counter), 1);
   std::cout << "Fetched and added from server: " << counter << std::endl;
   co_await qp->write_with_imm(remote_mr, buffer, sizeof(buffer), 1);
   co_await qp->compare_and_swap(counter_mr, &counter, sizeof(counter), 43, 4422);
...
int main(int argc, char* argv[])
{
   ...
   auto pd = std::make_shared<rdmapp::protected_domain>(device);
   auto cq = std::make_shared<rdmapp::completion_queue>(device);
   ...
   if (argc == 2) {
      ...
      std::cout << "Usage: " << argv[0] << " [port] for server and " << argv[0] << " [server_ip] [port] for client"
                << std::endl;
   }
   loop->close();
   looper.join();
