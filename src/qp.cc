...
namespace rdmapp
{
   ...
   qp::qp(uint16_t remote_lid, uint32_t remote_qpn, uint32_t remote_psn, std::shared_ptr<pd> pd, std::shared_ptr<cq> cq,
   ...
   qp::qp(uint16_t remote_lid, uint32_t remote_qpn, uint32_t remote_psn, std::shared_ptr<pd> pd,
          std::shared_ptr<cq> recv_cq, std::shared_ptr<cq> send_cq, std::shared_ptr<srq> srq)
   ...
   qp::qp(std::shared_ptr<rdmapp::pd> pd, std::shared_ptr<cq> cq, std::shared_ptr<srq> srq) : qp(pd, cq, cq, srq) {}

   qp::qp(std::shared_ptr<rdmapp::pd> pd, std::shared_ptr<cq> recv_cq, std::shared_ptr<cq> send_cq,
          std::shared_ptr<srq> srq)
   ...
   void qp::init()
   {
      ...
      try {
         check_rc(
            ::ibv_modify_qp(qp_, &(qp_attr), IBV_QP_STATE | IBV_QP_PORT | IBV_QP_ACCESS_FLAGS | IBV_QP_PKEY_INDEX),
   ...
   qp::send_awaitable::send_awaitable(std::shared_ptr<qp> qp, std::shared_ptr<local_mr> local_mr,
      ...
      : qp_(qp), local_mr_(local_mr), remote_mr_(remote_mr), compare_add_(compare), swap_(swap), opcode_(opcode)
   {}

   static inline struct ibv_sge fill_local_sge(const local_mr& mr)
   {
      struct ibv_sge sge = {};
   ...
   bool qp::send_awaitable::await_suspend(std::coroutine_handle<> h) noexcept
   {
      ...
      if (is_rdma()) {
         ...
         assert(remote_mr_.addr != nullptr);
         send_wr.wr.atomic.remote_addr = reinterpret_cast<uint64_t>(remote_mr_.addr);
   ...
   void qp::destroy()
   {
      ...
      if (auto rc = ::ibv_destroy_qp(qp_); rc != 0) [[unlikely]] {
         RDMAPP_LOG_ERROR("failed to destroy qp %p: %s", reinterpret_cast<void*>(qp_), strerror(errno));
      }
      else {
